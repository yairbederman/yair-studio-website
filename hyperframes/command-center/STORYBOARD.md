...
| 3 | 7.5–10.0s | **PEAK — the approval queue.** Everything dims except the APPROVAL column (2 waiting items, copper dots); ONE bounded copper ring bloom; check draws `scaleX 0 → 1` on item 1; caption = the safety line verbatim. |
...
Linear film + soft settle (Films 1–4 convention); the page restarts at 0.
Renders per locale: `command-center.{webm,mp4}` + `command-center-poster.png`
and `-he` variants → `public/videos/` (~1.5MB MP4 target). EN + HE from one
composition via the `lang` variable (see DESIGN.md "Locales").
