...
| 10.9–12.5 | **Human approval (compact peak)** | Everything dims (rack focus). Copper gate node scales up at the centre of the row block (960, 650), "Human approval" in the headline band (copper, 120px — spec v2), one bounded pulse, ✓ draws (complete at 12.05s). |
| 12.5–14.0 | **Workflow runs with less friction** | Dim lifts; closing headline (120px — EN on two lines, HE one line); rows settle with the three copper checks and the warm human row; soft settle for loop. |
...
| 5 | 11.8s | **APPROVAL** — dim + gate + ✓ (still drawing) + label in the band |
